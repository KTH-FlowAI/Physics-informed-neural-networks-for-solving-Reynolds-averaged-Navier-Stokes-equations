...
U_net = models.load_model('model_2.h5')
...
# res, diff = pinns.pred(xy_p)
ux_p = None #diff[:, 0].reshape(u.shape) 
uy_p = None #diff[:, 1].reshape(u.shape) 
vx_p = None #diff[:, 2].reshape(u.shape) 
vy_p = None #diff[:, 3].reshape(u.shape) 
#%%
plt.contourf(x, y, u)
plt.figure()
...
# %%
np.savez_compressed('./test_6', u_p = u_p, v_p = v_p, p_p = p_p, uv_p = uv_p, uu_p = uu_p, vv_p = vv_p, 
                    u = u, v = v, p = p, uv = uv, uu = uu, vv = vv,
                    x = x, y = y,
                    ux_p = ux_p, uy_p = uy_p, vx_p = vx_p, vy_p = vy_p)

err = np.savetxt('err.txt', np.array([e_u, e_v, e_p, e_uv, e_uu, e_vv]))
