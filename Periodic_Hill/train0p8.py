...
#%%
d = np.load('data/stsphill0p8.npz')
pos = d['pos']
scal = d['scal']
...
xy_f = np.concatenate((x[::10, ::10].reshape((-1, 1)),
                       y[::10, ::10].reshape((-1, 1))), axis = 1)
...
h_l = inp_U
for i in range(4):
    h_l = layers.Dense(10, activation = act, kernel_initializer = initializer)(h_l)
...
plt.plot(y[:, n], uv_p[:, n])
#%%
np.savez_compressed('./test', u_p = u_p, v_p = v_p, p_p = p_p
